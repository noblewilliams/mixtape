...
Future<ParsedExport> parseExportInIsolate(String path, ParseOptions options) {
  ...
  return _runCancellable(
    options.cancelToken,
    (events) => _spawnParse(path, options.timeZone, options.includePrivateSessions, events),
...
Future<ExportInventory> inspectExportInIsolate(String path, {CancelToken? cancelToken}) =>
...
Future<ParsedExport> _spawnParse(String path, String timeZone, bool includePrivateSessions, SendPort events) =>
    Isolate.run(
...
Future<ExportInventory> _spawnInspect(String path, SendPort events) =>
    Isolate.run(() => _inspect(path, events), debugName: 'spotify-export-inspector');

Future<ParsedExport> _parse(String path, String timeZone, bool includePrivateSessions, SendPort events) =>
    _withArchive(
      path,
      events,
...
Future<ExportInventory> _inspect(String path, SendPort events) =>
    _withArchive(path, events, (archive, token) => inspectExport(archive, cancelToken: token));
...
Future<T> _withArchive<T>(
  String path,
  SendPort events,
  Future<T> Function(ZipExportArchive archive, CancelToken token) body,
) async {
