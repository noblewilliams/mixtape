...
#### A1: fixture suite
- `fixtures/listening-exports/README.md` (the contract above, how to add a case, how the builder runs), `fixtures/listening-exports/src/<case>/` JSON definitions, `fixtures/listening-exports/build.mjs` (Node, deterministic: fixed entry mtimes, sorted entries) producing `fixtures/listening-exports/<case>/archive.zip` and `expected.json` (plus `expected.private-included.json` where the toggle matters), and a vitest in `server/test/fixtures/` or a root `fixtures/test` that rebuilds and asserts identical bytes.
